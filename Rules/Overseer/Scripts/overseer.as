...
{
	string configstr = "Rules/Overseer/my_ctf_vars.cfg";
	...
	s32 warmUpTimeSeconds = cfg.read_s32("warmup_time", 60);
	...
	s32 gameDurationMinutes = cfg.read_s32("game_time", 5);
	...
	this.triggerRemoveOverseer = true;
...
shared class CTFCore : RulesCore
{
	...
	void Update()
	{
		...
		{
			rules.SetGlobalMessage("Overseer phase; The overseer(s) are placing blueprints. Time left: {SEC}");
			rules.AddGlobalMessageReplacement("SEC", "" + ((ticksToStart / 30) + 1));
			ctf_spawns.force = true;
		}

		if ((rules.isIntermission() || rules.isWarmup()) && !getRules().get_bool("has_overseer"))  //CHECK IF TEAMS HAVE ENOUGH PLAYERS
		{
			gamestart = getGameTime();
			rules.set_u32("game_end_time", gamestart + gameDuration);
			rules.SetGlobalMessage("Waiting for at least 1 overseer to be chosen...");
			ctf_spawns.force = true;
		}
	...
	void CheckTeamWon()
	{
		...
		if(triggerRemoveOverseer)
		{
			triggerRemoveOverseer = false;
			CBitStream localparams;
			getRules().SendCommand(getRules().getCommandID("removeAllOverseer"), localparams);
		}
		int winteamIndex = -1;
		CTFTeamInfo@ winteam = null;
		s8 team_wins_on_end = -1;
		int value = getGameTime() % 300;
		if(value > 0 && value < 50 && triggerWinCheck)
		{
			...
			{
				winteamIndex = 0;
				CTFTeamInfo@ team = cast < CTFTeamInfo@ > (teams[0]);
				@winteam = team;
			}
		}
		else if(value >= 50)
		{
			triggerWinCheck = true;
		}
